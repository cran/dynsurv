				   ...
				   const ublas::matrix<double>& betaMat) const
	{
		...
		for (Size i = 0; i < this->N_; ++i) {
			...
			if (sumLeft == sumRight && sumLeft > 0) { //if (this->isObs_(i)) {
				...
				// Debug
				//std::cout << "lcsMat_i " << ublas::column(this->lcsMat_, i) << std::endl;
				//std::cout << "sumLeft: " << sumLeft << std::endl;
				//std::cout << "k      : " << k << std::endl;
				//std::cout << "lambda : " << lambda(k) << std::endl;
				//std::cout << "Xb     : " << ublas::prod(betaMat, ublas::row(this->pd_->X(), i)) << std::endl;
			}
